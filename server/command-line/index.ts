...
const program = new Command("chatik");
program
...
Config.setHome(process.env.CHATIK_HOME || Utils.defaultHome());
			...
			JSON.stringify(
				{
					private: true,
					description:
						"Packages for Chatik. Use `chatik install <package>` command to add a package.",
					dependencies: {},
				},
				null,
				"\t"
...
function verifyFileOwner() {
	...
	if (uid === 0) {
		log.warn(
			`You are currently running Chatik as root. ${colors.bold.red(
	...
	if (configStat && configStat.uid !== uid) {
		log.warn(
			"Config file owner does not match the user you are currently running Chatik as."
		);
		log.warn(
			"To prevent any issues, please run chatik commands " +
